...
function DashboardTab() {
    ...
    const { mode, loading, product, edithandle, deleteProduct, order, user } = context
    console.log(product);
                                ...
                                <div className=" flex justify-end">
                                    <button
                                        ...
                                        className="focus:outline-none text-white bg-pink-600 shadow-[inset_0_0_10px_rgba(0,0,0,0.6)] border hover:bg-pink-700 outline-0 font-medium rounded-lg text-sm px-5 py-2.5 mb-2" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} > <div className="flex gap-2 items-center">
                                            {loading ? <TailSpin height={25} color="white" /> : "Add Product"} <FaCartPlus size={20} />
                                        </div></button>
                                </div>
                                ...
                                {order.map((allorder, index) => {
                                    return (<table key={index} className="w-full text-sm text-left text-gray-500 dark:text-gray-400" >
                                        ...
                                        {allorder.cartItem.map((item, index) => {
                                            const { category, descripetion, imageUrl, price, title } = item;
